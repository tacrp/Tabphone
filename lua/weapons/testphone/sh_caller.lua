...
if CLIENT then
	...
	net.Receive("Tabphone_Call_HangUp_Recipient", function(len, ply)
...
if SERVER then
	...
	net.Receive("Tabphone_Call_Send", function(len, ply)
		...
		elseif !CallingEnt or !CallingEnt:IsValid() then
			Reason = TP_CALLCONFIRM_NONEXISTANT
		elseif CallingEnt == ply then
			Reason = TP_CALLCONFIRM_SMARTASS
		else
			local wow = { StartTime = CurTime(), Sender = ply, Recipient = CallingEnt, CallAccepted = false, Ringed = false }
	...
	hook.Add("Think", "Tabphone_Think", function()
		...
		for line, calldata in pairs(TABPHONE_LINES) do
			if (calldata.StartTime+5 <= CurTime()) and !calldata.CallAccepted then
				...
				continue
			elseif (calldata.StartTime+0.5 <= CurTime()) and !calldata.CallAccepted and !calldata.Ringed then
